#!/usr/bin/env python3
"""AST-based Django migration optimizer to replace legacy data migrations with no-ops."""
...
class RunPythonOptimizer(ast.NodeTransformer):
    ...
    def visit_Call(self, node):
        """Visit call nodes and replace migrations.RunPython when appropriate."""
            ...
            if node.func.value.id == 'migrations' and node.func.attr == 'RunPython':
                ...
                if code_kw:
                    ...
                    if isinstance(code_kw, ast.Attribute):
                        ...
                        isinstance(code_kw, ast.Call)
                        and isinstance(code_kw.func, ast.Name)
                        and code_kw.func.id == 'get_migration_func'
                    ):
                        if len(code_kw.args) > 1 and isinstance(
                            code_kw.args[1], ast.Constant
                        ):
                ...
                if not preserve_code:
                    ...
                    node.args = [noop_node]
                    node.keywords = [ast.keyword(arg='reverse_code', value=noop_node)]
        return node
                    ...
                    for target in class_node.targets:
                        if isinstance(target, ast.Name) and target.id == 'operations':
                            if isinstance(class_node.value, ast.List):
                                ...
                                for op in old_ops:
                                    if isinstance(op, ast.Call) and isinstance(
                                ...
                                print(
                                    f'  -> Report app operations optimized. Removed obsolete model operations. Reduced list from {len(old_ops)} to {len(new_ops)}.'
                                )
            break
                            ...
                            if isinstance(class_node.value, ast.List):
                                ...
                                for op in old_ops:
                                    ...
                                    if (
                                        ...
                                        and op.func.id == 'RemoveFieldOrSkip'
                                    ):
                                        continue
                                    ...
                                    if (
                                        isinstance(op, ast.Call)
                                        and isinstance(op.func, ast.Name)
                                        and op.func.id == 'AddFieldOrSkip'
                                    ):
                                        op.func = ast.Attribute(
                                            value=ast.Name(
                                                id='migrations', ctx=ast.Load()
                                            ),
                                            attr='AddField',
                                            ctx=ast.Load(),
                                        )
                                ...
                                class_node.value.elts = new_ops
                                print(
                                    '  -> Part block 2 operations optimized. Removed RemoveFieldOrSkip and converted AddFieldOrSkip to AddField.'
                                )
            break
...
def optimize_file(filepath):
    ...
    if '0097_partstocktakereport.fake_func' in content:
        content = content.replace(
            'part.migrations.0097_partstocktakereport.fake_func', 'fake_func'
        )
    ...
    if 'part' in filepath and '0061_' in filepath and '_squashed_' in filepath:
        ...
        tree.body = [
            ...
            if not (
                ...
                and node.name in {'RemoveFieldOrSkip', 'AddFieldOrSkip'}
            )
        ]
        print(
            '  -> Removed RemoveFieldOrSkip and AddFieldOrSkip class definitions from module body.'
        )
    ...
    if 'get_migration_func' in content:
        if not re.search(r'get_migration_func\s*\(', content):
            content = re.sub(
                r'def get_migration_func.*?return lambda apps, schema_editor: None',
                '',
                content,
                flags=re.DOTALL,
            )
...
def main():
    """Main entry point to run AST optimization on specified or discovered squashed migrations."""
    if len(sys.argv) > 1:
        optimize_file(sys.argv[1])
        return

    base_dir = 'src/backend/InvenTree'
    squashed_files = glob.glob(
        os.path.join(base_dir, '**/migrations/*_squashed_*.py'), recursive=True
    )
    ...
    if '--reset' in sys.argv and target_files:
        ...
        subprocess.run(
            ['git', 'checkout', '--', *target_files], shell=False, check=False
        )
