#!/usr/bin/env python3
"""Migration squasher helper tool to count migrations and resolve leading-digit syntax/loader errors."""
...
"""
    ...
    if app_name == 'part':
        content = content.replace(
            'part.migrations.0112_auto_20230525_1606.RemoveFieldOrSkip',
            'RemoveFieldOrSkip',
        )
        content = content.replace(
            'part.migrations.0112_auto_20230525_1606.AddFieldOrSkip', 'AddFieldOrSkip'
        )
...
def main():
    """Main entry point for migration analyzer and post-processor fix tool."""
    if len(sys.argv) > 1 and sys.argv[1] == 'fix':
        if len(sys.argv) < 4:
            print(
                'Error: fix command requires both <filepath> and <app_name> arguments.'
            )
    ...
    print(
        f'Detected {len(apps_info)} apps with a total of {total_files} migration files.\n'
    )

    print(f'{"App":<20} | {"Migration Count":<15} | {"Path"}')
    print('-' * 60)
    for app, info in sorted(
        apps_info.items(), key=lambda x: x[1]['count'], reverse=True
    ):
